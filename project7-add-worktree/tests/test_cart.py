...
def test_apply_tax():
    ...
    # Apply 10% tax: prices should become 110 and 220
    cart.apply_tax(10)
    
    assert abs(cart.items[0]["price"] - 110.00) < 0.001, f"Expected 110.00, got {cart.items[0]['price']}"
    assert abs(cart.items[1]["price"] - 220.00) < 0.001, f"Expected 220.00, got {cart.items[1]['price']}"


def test_apply_tax_total():
    cart = ShoppingCart()
    cart.add_item("Expensive", 1000.00, 1)
    
    # Apply 25% tax: price should become 1250
    cart.apply_tax(25)
    
    assert cart.items[0]["price"] == 1250.00, f"Expected 1250.00, got {cart.items[0]['price']}"
    assert cart.get_total() == 1250.00
